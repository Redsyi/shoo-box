...
void OutlineObject_float(float2 UV, float OutlineThickness, float NormalsSensitivity, out float Out)
{
	...
	uvSamples[0] = UV - float2(1, 1) * halfScaleFloor;
	uvSamples[1] = UV + float2(1, 1) * halfScaleCeil;
	uvSamples[2] = UV + float2(1 * halfScaleCeil, -1 * halfScaleFloor);
	uvSamples[3] = UV + float2(-1 * halfScaleFloor, 1 * halfScaleCeil);
